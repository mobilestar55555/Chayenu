...
function DBHelperService($q, $window, dbhelper){
    ...
    function createTables(){
        ...
        this.getDB().transaction(function(tx) {
            ...
            tx.executeSql("CREATE TABLE IF NOT EXISTS sections(ID INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, color TEXT, parsha_id INTEGER, order_key INTEGER, weekly INTEGER)", [], function(tx, result){
...
function SectionService($q, dbhelper){
    ...
    function bulkInsert(data){
        ...
        sql = "INSERT or REPLACE INTO sections (ID, title, color, parsha_id, order_key, weekly) VALUES(?, ?, ?, ?, ?, ?)";
        
        for(var i = 0; i < num_rows; i++){
            
            subValues = [data[i]['id'], data[i]['title'], data[i]['color'], data[i]['parsha_id'], data[i]['order'], data[i]['weekly']];
            values.push(subValues);
        }
...
function ApiService($q, $injector){
    ...
    function fetchData(last_date, api_service){
        ...
        $.ajax({
            type: "GET",
            url: "http://104.236.239.55/api/sync_data.php?last_synced="+last_date,
            dataType: "json",
